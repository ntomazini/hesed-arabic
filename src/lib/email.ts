 ...
 *   SMTP_FROM="Hesed Translation <noreply@hesed.com>"
 */
...
function getTransporter() {
  ...
  return {
    from: process.env.SMTP_FROM ?? `Hesed Translation <${user}>`,
...
export function buildResetSenhaEmail(name: string, resetLink: string) {
  const subject = 'Hesed Translation — Redefinição de senha'
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px;background:#f8fafc;">
      <div style="background:#1e3a5f;border-radius:12px 12px 0 0;padding:32px 32px 24px;text-align:center;">
        <div style="display:inline-block;background:white;border-radius:10px;padding:10px 18px;margin-bottom:16px;">
          <span style="color:#1e3a5f;font-weight:900;font-size:20px;letter-spacing:1px;">HT</span>
        </div>
        <h1 style="color:white;margin:0;font-size:22px;">Hesed Translation</h1>
        <p style="color:#93c5fd;margin:8px 0 0;font-size:14px;">Plataforma de Tradução</p>
      </div>
      <div style="background:white;border-radius:0 0 12px 12px;padding:32px;border:1px solid #e2e8f0;border-top:none;">
        ...
        <p style="color:#475569;font-size:15px;line-height:1.6;">
          Recebemos uma solicitação para redefinir a senha da sua conta na <strong>Hesed Translation</strong>.
...
export function buildSetupSenhaEmail(name: string, setupLink: string) {
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px;background:#f8fafc;">
      ...
      <div style="background:white;border-radius:0 0 12px 12px;padding:32px;border:1px solid #e2e8f0;border-top:none;">
        <p style="color:#1e293b;font-size:16px;">Olá, <strong>${name}</strong>!</p>
        <p style="color:#475569;font-size:15px;line-height:1.6;">
          Você foi adicionado(a) à plataforma de tradução <strong>Hesed Translation</strong>.
  ...
  `
  const text = `Olá, ${name}!\n\nVocê foi adicionado(a) à plataforma Hesed Translation.\n\nAcesse o link abaixo para criar sua senha:\n${setupLink}\n\nEste link é válido por 7 dias.`
  return { subject, html, text }
}
